  ...
  return withUsageExit(async () => {
    ...
    for (const r of targets) {
      writeBaseline(config, opts.cwd, makeBaseline({
        ...
        temperature: r.params?.temperature ?? config.defaults.temperature,
        maxTokens: r.params?.maxTokens ?? config.defaults.maxTokens,
        renderedPromptHash: r.renderedPromptHash,
        output: r.output, approvedBy: "cli", createdAt: opts.now,
      }));
    }
